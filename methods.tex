...
are implemented as ufuncs, so broadcasting applies also to expressions
such as \code{x + y * z}.
...
These can be used, for example, to as part of publishing to the PyPI website.
...
This means for most decisions everyone is entrusted with veto power.
...
% typically is correct right now, but do we want to write it so specifically?
...
generators deterministically. A key design element of a seed sequence is that
it can be used to spawn child \code{SeedSequence}s which are distinct from
their parent and all ancestors.
