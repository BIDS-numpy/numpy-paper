...
Various libraries build on and extend this functionality to further enrich the ecosystem of available tools.  For example, SciPy adds various mathematical, scientific, and engineering routines; Pandas provides indexed arrays, known as dataframes; xarray implements arrays with labeled axes; and Matplotlib generates publication-ready figures and visualizations.  Employing these low-level utilities are several widely used technique-specific libraries, including scikit-learn (machine learning), scikit-image (image processing), statmodels (statistics),  networkx (graphs and complex networks), and Napari (volumetric image visualization) \fixme{citations for all these}.  

Finally, domain-specific libraries and applications are built on this diverse foundation of tooling; these include astropy (astronomy) \fixme{citation}, NiPy (neuroimaging) \fixme{citation}, and pangeo (earth science) \fixme{citation}.

% ecosystem
% core: scipy, pandas, matplolib ;
% scikits: scikit-learn, scikit-image, statmodels, networkx
% domain specific: astropy (2 citations?), neuroscience (3 citations?), pangeo, napari, etc.
% Ipython / Jupyter
...
% teaching (neuroimaging paper / etc.?)
