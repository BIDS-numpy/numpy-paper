...
% FIXME: what are "specialized compilers and computing
% platforms"? Does Fortran need to be mentioned here?
...
NumPy is a community of practice with a culture of
...
Over three hundred of the most popular
...
It also makes it easy for other libraries to develop fast and
memory-efficient compiled code, usually in C or Fortran, that could manipulate
these arrays and pass them back to Python.
For users that require faster implementations but do not want to write lower-level
compiled code, a variety of tools exist
(e.g., Cython, Numba, Pythran) that transparently accelerate bottlenecks.
These tools allow programmers to express concepts using high-level array
programming without sacrificing efficiency.
In many cases, fast algorithms are already implemented by libraries in the
scientific Python ecosystem; these primitives simplify writing code in pure
Python.
