...
% this is the software engineering side of things
% as opposed to the interactive, exploration described above
...
NumPy functions are supported.  The protocols are implemented by at
least five libraries, including Dask, CuPy, xarray, and PyData/Sparse.
...
The protocols \textit{compose} well, and so it is possible to redeploy NumPy
code onto multi-GPU systems with minimal changes\cite{entschev2019}.
Working with the community, we have identified several issues that
prevent further adoption, and are refining interfaces to reduce
impedance mismatch.
...
Many fast algorithms are already implemented by libraries in the
scientific Python ecosystem.
Where extreme optimization is warranted, compiled languages such as Cython, Numba, Pythran, that
...
heralded the current era of widespread use of Python for scientific
computing. % This and the "What began as" sentence below are more or less interchangeable.
...
%% v TO ADD v
%%
%% Over the next decade, the computational landscape will change
%% drastically.
%% - AI / data science mature
%% - hardware / Moore's law, specialized devices
%% - more users (diversity + numbers)
%% - scale / type of data; scientific data, lightsheet microscopes, LSST
%% - new ideas come from other languages like Julia, Rust, ...; JIT improvements
%% NumPy is poised to tackle these challenges, and work continues.
%% - Major refactoring of code
%% - Governance
%% - Process (NEP, roadmap, how we handle issues on GH, etc.)
%% - Protocols: will be extending that work
%% - Funding (some recent evidence that that is improving → grants, or
%% Moore/Sloan/CZI) CZI EOSS
%% - Redesigning website and documentation to be more friendly, easy to
%% navigate
%% - Next generation of developers (grad students) to sign on
%%

NumPy's is no longer just the foundational array library underlying
...
array types and technologies in Python.
