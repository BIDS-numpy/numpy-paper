...
Other array-aware functions, such as \code{maximum}, perform
...
largest supercomputers \fixme{citation needed} and yields performance
...
% smooth experience/intengration from high level <-> low level?
...
\begin{figure*}
  ...
  \caption{\textbf{Interoperability.} Through its array protocol, the NumPy API can operate on various types of array objects, including those written outside of NumPy. In this example, NumPy's \code{sum} function is called on a NumPy and a dask array, respectively. The call gets succeeds by dispatching to the appropriate library implementation, and results in different output array types.}\label{fig:duck-arrays}
\end{figure*}
