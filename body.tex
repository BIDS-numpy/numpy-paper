...
NumPy provides in-memory, multidimensional, homogeneously typed (i.e.,
single pointer and strided) arrays on CPUs.  It runs on machines
...
In recentl years, there has been a rapid growth in the development of
specialized array processing hardware, including graphics processing
...
the cloud.  Due to its in-memory data model, NumPy is unable to
utilize such specialized hardware and storage.  However, both
distributed data and the parallel execution of GPUs and TPUs map well
...
But exploring new ways of working with array is experimental by nature
and, in fact, several promising libraries---such as Caffe, Theano, and
biggus---have already ceased development. And each time that a user
decides to try a new technology or backend library, they have to
...
\begin{figure*}
  ...
  \caption{\textbf{Interoperability.} Through its array protocol, the NumPy API
...
As an example, consider the arrays shown in ...
\fixme{this part to be fleshed out to match the code in the figure}
