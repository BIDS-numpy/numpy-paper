...
\fixme{Up to here we were talking about arrays, now we introduce array programming.
Need to better explain vectorization in array programming.
This is perhaps the essential thing about it so we should make it slightly
more explicit.  It is just shorter to type, but it is easier to think
about. }
To complement the array syntax, NumPy includes functions that perform
vectorized calculations on arrays, including arithmetic, statistics, and
trigonometry.
...
When performing a vectorized operation (such as addition) on two arrays, one
expects that these arrays should have the same shape.
% might expect if you are coming from other languages
% wouldn't expect if you were doing math
% scalar vector multiplication, scalar matrix multiplication, etc.
...
The brevity and expressive power of NumPy's array programming syntax combined with 
...
SciPy provides many fundamental algorithms for scientific computing on arrays
...
% say something about this already being a good foundation comparable to
% other scientific computing languages.

The scientific Python ecosystem (Fig.~\ref{fig:ecosystem}) builds on top of
this foundation providing many, widely used technique-specific
...
Working interactively with their data, users discover through experimentation
bits of code that they stitch together into imperative or functional programs.
This rich and productive environment has made the scientific Python ecosystem popular for scientific
research.
...
\code{scipy.optimize} subpackage for handling general optimization tasks.
...
is used throughout \code{eht-imaging},
...
Tools for analyzing data from gravitational wave observatories are
provided by the \code{pycbc} package, which was used in the first
detection of gravitational waves \cite{abbott2016observation} and in 
on-going analysis of data from LIGO and Virgo.
\code{pycbc} also make extensive use of the scientific Python ecosystem.
The time-series data from the interferometers are stored in NumPy arrays,
while the \code{scipy.signal} subpackage is used to construct and apply filters to the
data.
...
% similar tools exist in physics, chemistry, biology, earth sciences,
% engineering
% this means there is an extraordinary need for stability and reliability
% this conflicts with competing need to innovate
% resolving this tension requires balancing trade-offs
% this lead to a proliferation of arrays  
...
To facilitate \emph{interoperability}, the NumPy provides
...
% need to add a bit of clarity on what "enable and interact naturally with different libraries" means.
%
%  1. I expect multiple user dtypes can interact (naturally); 2. libraries such
%  as sklearn (or dask) might "just work" with e.g. physical Units (and
%  Categoricals, which they probably really would like to use e.g. for labels);
%  3. Things being inside a dtype rather than subclass is a better user
%  experience.
