...
$4 \times 3$, where each element occupies 8 bytes in memory.
...
The strides of that array are therefore $(24, 8)$.
...
\begin{figure}
  ...
  \caption{\textbf{Reductions.} Reduction operations act along one or more axes. In this example, a three-dimensional array is shown to be summed along various single axes to produce two-dimensional results, or along two axes consecutively to produce a one-dimensional result. \fixme{May want to fix to have right-hand axis system.}
   }
...
%(e.g., summation across columns, see Fig.~\ref{fig:reductions}) products, and %so forth, are
%defined.
...
array programming.
Exposing these array programming primitives in Python---an interpreted
language---creates an ideal environment for exploratory data analysis where
users may inspect, manipulate, and visualize their data
...
\fixme{NumPy is now omnipresent.  Many packages depend on it.}


- new hardware platforms; large scales of data; computational barriers; ...

\fixme{arrays are a nice idea in principle, but not all problems are that easily
solved; it breaks down when you have very large data sets, when your data
doesn't live on a single machine, when you have to use specialized hardware, or
when several resources need to be combined to solve a single array computation}

This has lead to a proliferation of arrays (Fig.~\ref{fig:array-proliferation}).
...
NumPy's API has become ubiquitous in scientific computation, but NumPy itself
can never hope to satisfy every single specialized need of the community.
The advent of new technologies, such as GPUs, create opportunities that should be
exploited, but are out of scope for NumPy development.
Still, the widespread adoption of the NumPy API is valuable, as it lowers the
barrier to entry and provides the wider community with a stable array programming interface.
...
There has been a steady increase in the number of external libraries that mimic
NumPy, i.e. provide arrays and a NumPy-like API for manipulating them.
...
NumPy, in turn, dispatches operations to the originating library, as required.  Early versions of these protocols have been successfully deployed.
...
Lastly, NumPy has benefited greatly from a well-chosen API.
Scientific and numerical programming needs to be fast, and scale to very large
datasets.
The NumPy API defines a simple wrapper for data in memory so it can be
represented as a one- or multi-dimensional array.
The simplicity of this wrapper has made it very successful as a standard way of
representing arrays in memory, and has made it relatively easy for other
libraries to develop fast and memory-efficient compiled code, usually in C or
Fortran, that can manipulate these arrays and pass them back to Python.
