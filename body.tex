...
other specialized hardware for performance reasons.

% Ralf: should mention that there are typically utilities to convert to and from numpy arrays
...
% Ralf: may want to add something like:
...
% Ralf: I am not sure where this goes, but it would be good to have something
% about dtype limitation in this section and i also think there should
% be some discussion of missing values somewhere
% (it is mentioned below, but not discussed)
While it supports a wide range of datatypes, it does not have good support
for missing values. % may be worth saying a few sentences about missing values
% if we want to discuss Pandas, perhaps we could do so here (i.e, missing values)
