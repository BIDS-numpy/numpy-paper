...
narrative computational notebooks.
...
\begin{figure}
  ...
  \caption{\textbf{Scientific Python Ecosystem.}
   ...
   This means that other array implementations are easily accessible to
   the ecosystem if they use the NumPy API and array protocol (Fig.~\ref{fig:array-protocol}).
  }
  \label{fig:ecosystem}
\end{figure}
...
\begin{figure*}
  ...
  \caption{\textbf{NumPy API and Array Protocol.} Through its array protocol, the NumPy API
     ...
     of NumPy. In this example, NumPy's \code{mean} function is called on a a Dask
