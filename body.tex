...
While NumPy remains the fundamental tool of choice for a large portion
of applications the growth of the computational landscape and methods
cannot be supported within a single library.
However, the NumPy API is a preferred language for array computation
and often one of the first tools learned.
...
We believe that improving the interoperability and moving NumPy forward
to serve new arising needs where possible is the best way to balance
the need for stability on the one hand while supporting the growth of
specialized solutions and new tools on the other.
...
NumPy also aims to improve our \emph{extensibility}.
